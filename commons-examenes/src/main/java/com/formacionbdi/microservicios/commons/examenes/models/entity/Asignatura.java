...
public class Asignatura {
	...
	@JsonIgnoreProperties(value = {"hijos", "handler", "hibernateLazyInitializer"}) // PAra que no se genere un bucle infinito
	@ManyToOne(fetch = FetchType.LAZY)
	private Asignatura padre;
	
	@JsonIgnoreProperties(value = {"padre", "handler", "hibernateLazyInitializer"}, allowSetters = true)
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "padre", cascade = CascadeType.ALL) // Si borramos un padre tambien borramos al hijo (Creamos en cascada)
